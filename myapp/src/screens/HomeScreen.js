...
import hotDealsImage from "../../assets/hot_deals.jpg";
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (response.success) {
          ...
          const categoryList = response.productCategories.map((category) => ({
            id: category.id,
            name: category.title,
    ...
    <>
      <SafeAreaView style={styles.container}>
        <ScrollView ref={scrollViewRef}>
          ...
          <ScrollView  horizontal showsHorizontalScrollIndicator={false} style={styles.srcListItemContainer} >
          <View style={styles.srcListItemInnerContainer}>
            {categories.map((item, index) => (
              
              <Pressable
              onPress={() => handlePressCategory(item?.name) }
               key={index} style={styles.listItemContainer}>
                <Image
